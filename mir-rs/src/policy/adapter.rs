//! Adapter that bridges the `mir` WindowManagementPolicy to the internal FFI policy bridge.

use crate::sys::PolicyBridge;
use crate::sys::ffi;
...
impl<P: WindowManagementPolicy> PolicyBridge for PolicyBridgeAdapter<P> {
    ...
    fn handle_activate_window(&mut self, window_info: &ffi::MiralWindowInfo) {
        let info = Self::make_window_info(window_info);
        self.policy.handle_activate_window(&info);
    }
