...
// run). Atomic so it can be safely set from the start callback thread and read
// from the input/policy thread without a mutex.
namespace {
...
static rust::String
encode_input_shape(std::vector<mir::geometry::Rectangle> const &rects) {
  std::string result;
...
static std::vector<mir::geometry::Rectangle>
decode_input_shape(rust::String const &encoded) {
  ...
  while (pos < str.size()) {
    ...
    rects.push_back(mir::geometry::Rectangle{{values[0], values[1]},
                                             {values[2], values[3]}});
  }
  return rects;
}
...
/// `parent_id` must be resolved by the caller since it requires the tools
/// registry.
static WindowSpecData spec_to_ffi(miral::WindowSpecification const &spec,
                                  bool has_parent, uint64_t parent_id) {
  return WindowSpecData{
      spec.top_left().has_value(),
      spec.top_left().has_value() ? to_point(spec.top_left().value())
                               : Point{0, 0},
      spec.size().has_value(),
      spec.size().has_value() ? to_size(spec.size().value()) : Size{0, 0},
      spec.state().has_value(),
      spec.state().has_value() ? static_cast<int32_t>(spec.state().value()) : 0,
      spec.type().has_value(),
      spec.type().has_value() ? static_cast<int32_t>(spec.type().value()) : 0,
      spec.name().has_value(),
      spec.name().has_value() ? rust::String(spec.name().value())
                           : rust::String(""),
      has_parent,
      parent_id,
      spec.min_width().has_value(),
      spec.min_width().has_value() ? spec.min_width().value().as_int() : 0,
      spec.min_height().has_value(),
      spec.min_height().has_value() ? spec.min_height().value().as_int() : 0,
      spec.max_width().has_value(),
      spec.max_width().has_value() ? spec.max_width().value().as_int() : 0,
      spec.max_height().has_value(),
      spec.max_height().has_value() ? spec.max_height().value().as_int() : 0,
      spec.depth_layer().has_value(),
      spec.depth_layer().has_value()
          ? static_cast<int32_t>(spec.depth_layer().value())
          : 0,
      spec.focus_mode().has_value(),
      spec.focus_mode().has_value()
          ? static_cast<int32_t>(spec.focus_mode().value())
          : 0,
      spec.shell_chrome().has_value(),
      spec.shell_chrome().has_value()
          ? static_cast<int32_t>(spec.shell_chrome().value())
          : 0,
      spec.preferred_orientation().has_value(),
      spec.preferred_orientation().has_value()
          ? static_cast<int32_t>(spec.preferred_orientation().value())
          : 0,
      spec.confine_pointer().has_value(),
      spec.confine_pointer().has_value()
          ? static_cast<int32_t>(spec.confine_pointer().value())
          : 0,
      spec.attached_edges().has_value(),
      spec.attached_edges().has_value()
          ? static_cast<uint32_t>(spec.attached_edges().value())
          : 0u,
      spec.visible_on_lock_screen().has_value(),
      spec.visible_on_lock_screen().has_value()
          ? spec.visible_on_lock_screen().value()
          : false,
      // --- New fields ---
      spec.output_id().has_value(),
      spec.output_id().has_value() ? spec.output_id().value() : 0,
      spec.aux_rect().has_value(),
      spec.aux_rect().has_value() ? to_rectangle(spec.aux_rect().value())
                               : Rectangle{{0, 0}, {0, 0}},
      spec.placement_hints().has_value(),
      spec.placement_hints().has_value()
          ? static_cast<uint32_t>(spec.placement_hints().value())
          : 0u,
      spec.window_placement_gravity().has_value(),
      spec.window_placement_gravity().has_value()
          ? static_cast<uint32_t>(spec.window_placement_gravity().value())
          : 0u,
      spec.aux_rect_placement_gravity().has_value(),
      spec.aux_rect_placement_gravity().has_value()
          ? static_cast<uint32_t>(spec.aux_rect_placement_gravity().value())
          : 0u,
      spec.aux_rect_placement_offset().has_value(),
      spec.aux_rect_placement_offset().has_value()
          ? to_displacement(spec.aux_rect_placement_offset().value())
          : Displacement{0, 0},
      spec.width_inc().has_value(),
      spec.width_inc().has_value() ? spec.width_inc().value().as_int() : 0,
      spec.height_inc().has_value(),
      spec.height_inc().has_value() ? spec.height_inc().value().as_int() : 0,
      spec.min_aspect().has_value(),
      spec.min_aspect().has_value() ? spec.min_aspect().value().width : 0u,
      spec.min_aspect().has_value() ? spec.min_aspect().value().height : 0u,
      spec.max_aspect().has_value(),
      spec.max_aspect().has_value() ? spec.max_aspect().value().width : 0u,
      spec.max_aspect().has_value() ? spec.max_aspect().value().height : 0u,
      spec.input_shape().has_value(),
      spec.input_shape().has_value()
          ? encode_input_shape(spec.input_shape().value())
          : rust::String(""),
      spec.input_mode().has_value(),
      spec.input_mode().has_value()
          ? static_cast<int32_t>(spec.input_mode().value())
          : 0,
      spec.exclusive_rect().has_value(),
      spec.exclusive_rect().has_value() && spec.exclusive_rect().value().has_value(),
      (spec.exclusive_rect().has_value() && spec.exclusive_rect().value().has_value())
          ? to_rectangle(spec.exclusive_rect().value().value())
          : Rectangle{{0, 0}, {0, 0}},
      spec.ignore_exclusion_zones().has_value(),
      spec.ignore_exclusion_zones().has_value()
          ? spec.ignore_exclusion_zones().value()
          : false,
      spec.application_id().has_value(),
      spec.application_id().has_value()
          ? rust::String(spec.application_id().value())
          : rust::String(""),
      spec.server_side_decorated().has_value(),
      spec.server_side_decorated().has_value()
          ? spec.server_side_decorated().value()
          : false,
      spec.tiled_edges().has_value(),
      spec.tiled_edges().has_value()
          ? static_cast<uint32_t>(spec.tiled_edges().value().value())
          : 0u,
      spec.alpha().has_value(),
      spec.alpha().has_value() ? spec.alpha().value() : 0.0f,
...
static void ffi_to_spec(WindowSpecData const &data,
                        miral::WindowSpecification &out, MiralTools *tools) {
  ...
  if (data.has_preferred_orientation)
    out.preferred_orientation() =
        static_cast<MirOrientationMode>(data.preferred_orientation);
  if (data.has_confine_pointer)
    out.confine_pointer() =
        static_cast<MirPointerConfinementState>(data.confine_pointer);
  if (data.has_attached_edges)
    out.attached_edges() =
        static_cast<MirPlacementGravity>(data.attached_edges);
  ...
  if (data.has_placement_hints)
    out.placement_hints() =
        static_cast<MirPlacementHints>(data.placement_hints);
  if (data.has_window_placement_gravity)
    out.window_placement_gravity() =
        static_cast<MirPlacementGravity>(data.window_placement_gravity);
  if (data.has_aux_rect_placement_gravity)
    out.aux_rect_placement_gravity() =
        static_cast<MirPlacementGravity>(data.aux_rect_placement_gravity);
  if (data.has_aux_rect_placement_offset)
    out.aux_rect_placement_offset() =
        from_displacement(data.aux_rect_placement_offset);
  ...
  if (data.has_min_aspect)
    out.min_aspect() = miral::WindowSpecification::AspectRatio{
        data.min_aspect_width, data.min_aspect_height};
  if (data.has_max_aspect)
    out.max_aspect() = miral::WindowSpecification::AspectRatio{
        data.max_aspect_width, data.max_aspect_height};
  if (data.has_input_shape)
    out.input_shape() = decode_input_shape(data.input_shape);
  if (data.has_input_mode)
    out.input_mode() =
        static_cast<miral::WindowSpecification::InputReceptionMode>(
            data.input_mode);
  if (data.has_exclusive_rect) {
    if (data.exclusive_rect_is_set)
      out.exclusive_rect() = std::optional<mir::geometry::Rectangle>{
          from_rectangle(data.exclusive_rect)};
    else
      out.exclusive_rect() = std::optional<mir::geometry::Rectangle>{};
  }
  ...
  if (data.has_application_id)
    out.application_id() =
        std::string(data.application_id.data(), data.application_id.size());
  if (data.has_server_side_decorated)
    out.server_side_decorated() = data.server_side_decorated;
  if (data.has_tiled_edges)
    out.tiled_edges() =
        mir::Flags<MirTiledEdge>{static_cast<unsigned int>(data.tiled_edges)};
  if (data.has_alpha)
...
void MiralTools::unregister_window(uint64_t id) { window_registry.erase(id); }
...
public:
  ...
  place_new_window(miral::ApplicationInfo const &app_info,
    ...
    // Resolve parent ID
    auto has_parent = requested_specification.parent().has_value();
    ...
    auto spec_data =
        spec_to_ffi(requested_specification, has_parent, parent_id);
  ...
  void handle_modify_window(
    ...
    auto mod_has_parent = modifications.parent().has_value();
  ...
  void handle_activate_window(miral::WindowInfo &window_info) override {
    tools_->register_window(window_info.window());
    MiralWindowInfo info_wrap{window_info};
    rust_policy_handle_activate_window(*holder_, info_wrap);
  }
  ...
  bool handle_touch_event(MirTouchEvent const *event) override {
    ...
    TouchEventInfo info{
        mir_touch_event_point_count(event) > 0
            ? static_cast<int32_t>(mir_touch_event_id(event, 0))
            : 0,
        mir_touch_event_point_count(event) > 0
            ? static_cast<int32_t>(mir_touch_event_action(event, 0))
            : 0,
        static_cast<uint32_t>(mir_touch_event_modifiers(event)),
        mir_touch_event_point_count(event) > 0
            ? mir_touch_event_axis_value(event, 0, mir_touch_axis_x)
            : 0.0f,
        mir_touch_event_point_count(event) > 0
            ? mir_touch_event_axis_value(event, 0, mir_touch_axis_y)
            : 0.0f,
  ...
  void handle_request_move(miral::WindowInfo &window_info,
  ...
  void handle_request_resize(miral::WindowInfo &window_info,
    ...
    tools_->register_window(window_info.window());
    MiralWindowInfo info_wrap{window_info};
    rust_policy_handle_request_resize(*holder_, info_wrap,
                                      static_cast<int32_t>(edge));
  }

  auto confirm_inherited_move(miral::WindowInfo const &window_info,
    ...
    return mir::geometry::Rectangle{from_point(result.top_left),
                                    from_size(result.size)};
  }

  auto
  confirm_placement_on_display(miral::WindowInfo const &window_info,
  ...
  void advise_begin() override { rust_policy_advise_begin(*holder_); }

  void advise_end() override { rust_policy_advise_end(*holder_); }
  ...
  void advise_state_change(miral::WindowInfo const &window_info,
                           MirWindowState state) override {
    MiralWindowInfo info_wrap{window_info};
    rust_policy_advise_state_change(*holder_, info_wrap,
                                    static_cast<int32_t>(state));
  }

  void advise_move_to(miral::WindowInfo const &window_info,
                      mir::geometry::Point top_left) override {
    MiralWindowInfo info_wrap{window_info};
    rust_policy_advise_move_to(*holder_, info_wrap, to_point(top_left));
  }

  void advise_resize(miral::WindowInfo const &window_info,
                     mir::geometry::Size const &new_size) override {
    MiralWindowInfo info_wrap{window_info};
  ...
  void advise_output_update(miral::Output const &updated,
                            miral::Output const &original) override {
  ...
  void advise_removing_from_workspace(
    ...
    for (auto const &w : windows)
      ids.push_back(window_to_id(w));
    rust_policy_advise_removing_from_workspace(*holder_,
                                               {ids.data(), ids.size()});
  }
...
int32_t miral_runner_run_with_config(
  ...
  for (auto const &desc : config_options) {
    ...
    miral::ConfigurationOption config_opt =
      ...
      case 1: // double with default
        return miral::ConfigurationOption{
            [cb_id](double value) {
              rust_config_callback_double(cb_id, value);
            },
            opt_name, opt_desc, desc.default_double};
      case 2: // string with default
      {
        std::string default_str(desc.default_string.data(),
                                desc.default_string.size());
        return miral::ConfigurationOption{
            [cb_id](std::string const &value) {
              rust_config_callback_string(
                  cb_id, rust::Str(value.data(), value.size()));
            },
      ...
      case 4: // flag (presence-only)
        return miral::ConfigurationOption{
            std::function<void(bool)>{[cb_id](bool is_set) {
              rust_config_callback_flag(cb_id, is_set);
            }},
            opt_name, opt_desc};
      case 5: // optional int
        return miral::ConfigurationOption{
            [cb_id](std::optional<int> const &value) {
              rust_config_callback_optional_int(
                  cb_id, value.has_value(), value.has_value() ? value.value() : 0);
            },
            opt_name, opt_desc};
      case 6: // optional string
        return miral::ConfigurationOption{
            [cb_id](std::optional<std::string> const &value) {
              if (value.has_value()) {
                auto const &str = value.value();
                rust_config_callback_optional_string(
                    cb_id, true, rust::Str(str.data(), str.size()));
              } else {
                rust_config_callback_optional_string(cb_id, false,
                                                     rust::Str("", 0));
              }
            },
            opt_name, opt_desc};
      case 7: // optional bool
        return miral::ConfigurationOption{
            [cb_id](std::optional<bool> const &value) {
              rust_config_callback_optional_bool(cb_id, value.has_value(),
      ...
      case 8: // multi-value string list
        return miral::ConfigurationOption{
            [cb_id](std::vector<std::string> const &values) {
              ...
              rust_config_callback_multi(
                  cb_id, {rust_values.data(), rust_values.size()});
            },
            opt_name, opt_desc};
      default:
        // Unknown type — create a no-op flag option as fallback
        return miral::ConfigurationOption{
            std::function<void(bool)>{[](bool) {}}, opt_name, opt_desc};
      }
    }();
  ...
  // External client launcher (special — needs start/stop callbacks for the
  // global pointer)
  if (runner.has_external_launcher) {
    runner.options.push_back(runner.external_launcher);
    runner.inner->add_start_callback(
        [&runner]() { g_active_launcher.store(&runner.external_launcher); });
    runner.inner->add_stop_callback([]() { g_active_launcher.store(nullptr); });
  }

  if (runner.has_magnifier) {
    runner.inner->add_start_callback(
        [&runner]() { g_active_magnifier.store(&runner.magnifier); });
...
void miral_runner_stop(MiralRunner &runner) { runner.inner->stop(); }

void miral_runner_add_decorations(MiralRunner &runner, int32_t mode) {
  switch (mode) {
  case 1:
    runner.options.push_back(miral::Decorations::prefer_csd());
    break;
  case 2:
    runner.options.push_back(miral::Decorations::prefer_ssd());
    break;
  case 3:
    runner.options.push_back(miral::Decorations::always_ssd());
    break;
  case 4:
    runner.options.push_back(miral::Decorations::always_csd());
    break;
  default:
    runner.options.push_back(miral::Decorations::prefer_csd());
    break;
...
void miral_runner_add_wayland_extensions(
    MiralRunner &runner, rust::Slice<const rust::String> enabled,
...
void miral_runner_add_idle_listener(MiralRunner &runner) {
  ...
  idle_listener.on_dim([]() { rust_on_idle_dim_callback(); })
...
void miral_runner_add_session_lock_listener(MiralRunner &runner) {
  runner.options.push_back(
      miral::SessionLockListener{[]() { rust_on_session_lock_callback(); },
                                 []() { rust_on_session_unlock_callback(); }});
}

void miral_runner_add_magnifier(MiralRunner &runner, float magnification,
                                int32_t width, int32_t height, bool enabled) {
...
void miral_runner_add_bounce_keys(MiralRunner &runner, bool enabled) {
  runner.options.push_back(enabled ? miral::BounceKeys::enabled()
                                   : miral::BounceKeys::disabled());
}

void miral_runner_add_slow_keys(MiralRunner &runner, bool enabled) {
  runner.options.push_back(enabled ? miral::SlowKeys::enabled()
                                   : miral::SlowKeys::disabled());
}

void miral_runner_add_sticky_keys(MiralRunner &runner, bool enabled) {
  runner.options.push_back(enabled ? miral::StickyKeys::enabled()
                                   : miral::StickyKeys::disabled());
}

void miral_runner_add_mousekeys(MiralRunner &runner, bool enabled) {
  runner.options.push_back(enabled ? miral::MouseKeysConfig::enabled()
                                   : miral::MouseKeysConfig::disabled());
}

void miral_runner_add_locate_pointer(MiralRunner &runner, bool enabled) {
  runner.options.push_back(enabled ? miral::LocatePointer::enabled()
                                   : miral::LocatePointer::disabled());
}

void miral_runner_add_cursor_theme(MiralRunner &runner, rust::Str theme) {
  runner.options.push_back(
      miral::CursorTheme{std::string(theme.data(), theme.size())});
}
...
void miral_runner_add_init_callback(MiralRunner &runner) {
  runner.options.push_back(
      miral::AddInitCallback{[]() { rust_on_init_callback(); }});
}

void miral_runner_add_terminator(MiralRunner &runner) {
  // miral::SetTerminator was removed in miral 6.0. The replacement is a signal
  // handler registered directly on the MirRunner; handlers registered before
  // the server starts are backlogged and applied once the main loop exists.
  runner.inner->register_signal_handler(
      {SIGINT, SIGTERM},
      [](int signal) { rust_on_terminator_callback(signal); });
}
...
void miral_runner_register_start_callback(MiralRunner &runner) {
  runner.inner->add_start_callback([]() { rust_on_start_callback(); });
}

void miral_runner_register_stop_callback(MiralRunner &runner) {
  runner.inner->add_stop_callback([]() { rust_on_stop_callback(); });
}
...
WindowInfoSnapshot miral_window_info_snapshot(MiralWindowInfo const &info) {
  ...
  return WindowInfoSnapshot{
      ...
      has_excl,
      has_excl_value,
      has_excl ? to_rectangle(excl.value())
               : Rectangle{Point{0, 0}, Size{0, 0}},
      i.ignore_exclusion_zones(),
      clip.has_value(),
      clip.has_value() ? to_rectangle(clip.value())
                    : Rectangle{Point{0, 0}, Size{0, 0}},
...
ApplicationInfoSnapshot
miral_app_info_snapshot(MiralApplicationInfo const &info) {
  auto const &i = info.inner;
...
void miral_tools_ask_client_to_close_by_id(MiralTools &tools,
                                           uint64_t window_id) {
  if (auto const *w = tools.lookup_window(window_id))
    tools.inner.ask_client_to_close(*w);
}

void miral_tools_modify_window_by_id(MiralTools &tools, uint64_t window_id,
                                     WindowSpecData const &spec) {
...
void miral_tools_select_active_window_by_id(MiralTools &tools,
                                            uint64_t window_id) {
  if (auto const *w = tools.lookup_window(window_id))
    tools.inner.select_active_window(*w);
}

std::unique_ptr<MiralWindow> miral_tools_window_at(MiralTools const &tools,
                                                   Point point) {
...
WindowInfoSnapshot miral_tools_info_for_window_id(MiralTools const &tools,
                                                  uint64_t window_id) {
  auto const *w = tools.lookup_window(window_id);
  if (!w) {
...
void miral_tools_swap_tree_order_by_id(MiralTools &tools, uint64_t window_id_a,
                                       uint64_t window_id_b) {
...
void miral_tools_send_tree_to_back_by_id(MiralTools &tools,
                                         uint64_t window_id) {
  if (auto const *w = tools.lookup_window(window_id))
...
rust::Vec<uint64_t> miral_tools_all_window_ids(MiralTools const &tools) {
  ...
  const_cast<miral::WindowManagerTools &>(tools.inner)
      .for_each_application([&](miral::ApplicationInfo &app_info) {
...
Rectangle miral_tools_place_and_size_for_state(MiralTools const &tools,
                                               uint64_t window_id,
                                               int32_t new_state,
  ...
  auto result_top_left = spec.top_left().has_value() ? spec.top_left().value()
                                                  : from_point(rect.top_left);
  auto result_size =
      spec.size().has_value() ? spec.size().value() : from_size(rect.size);
