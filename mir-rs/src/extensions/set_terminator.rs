...
///
/// The handler is invoked with the signal number when the server receives
/// `SIGINT` or `SIGTERM`.
///
/// Note that a handler registered here replaces the default shutdown
/// behaviour, so it is normally responsible for stopping the server itself.
